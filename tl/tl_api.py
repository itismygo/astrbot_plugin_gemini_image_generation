...
class GeminiAPIClient:
        ...
        async with session.post(
            ...
            if response.status == 200:
                ...
                if api_type == "google":
                    ...
                    return await self._parse_openai_response(response_data, session, url)
    ...
    async def _parse_openai_response(
        self, response_data: dict, session: aiohttp.ClientSession, api_url: str = None
    ) -> tuple[list[str], list[str], str | None, str | None]:
        """解析 OpenAI API 响应
        ...
        Args:
            ...
            session: aiohttp 会话
            api_url: API 请求 URL（用于构建相对路径的完整 URL）
        """

        # grok2api 适配：从 api_url 提取 base_url
        api_base = None
        if api_url:
            ...
            for candidate_url in image_candidates:
                ...
                if isinstance(candidate_url, str) and candidate_url.startswith(
                    ...
                    if candidate_url.startswith("/") and not candidate_url.startswith("//"):
                        if api_base:
                            full_url = urllib.parse.urljoin(api_base, candidate_url)
                    ...
                    if candidate_url.startswith("http://") or candidate_url.startswith(
                        ...
                        # grok2api 适配：验证 URL 不是不完整的（如 /images/ 结尾没有文件名）
                        parsed_url = urllib.parse.urlparse(candidate_url)
                        if parsed_url.path.rstrip('/') in ('', '/images'):
                            logger.warning(f"[grok2api 适配] 跳过不完整的图片 URL: {candidate_url}")
                            continue
