...
- **降级处理**: 自动处理参数兼容性问题
- **缓存机制**: 避免重复下载头像，提升性能
- **表情包智能缓存**: 自动保存所有收到的图片并建立索引，完美支持 QQ 商店表情包等特殊图片
...
插件内置了**表情包智能缓存系统**，能够自动保存所有收到的图片并建立 URL 索引。这解决了 QQ 商店表情包、付费表情等特殊图片无法直接通过 URL 获取的问题。

### 工作原理

1. **自动监听**: 插件会监听所有消息中的图片（包括引用消息）
2. **智能保存**: 优先通过本地缓存获取，失败则从 URL 下载
3. **索引建立**: 为每张图片的 URL 建立哈希索引，方便后续快速查找
4. **无缝使用**: 当你引用一张表情包进行生图时，插件会自动从本地缓存获取
...
```
{AstrBot数据目录}/plugin_data/astrbot_plugin_gemini_image_generation/sticker_cache/
├── sticker_20241206_123456_789.jpg  # 缓存的图片
├── sticker_20241206_123457_012.png
└── .url_index.json                   # URL 索引文件
```

### 使用示例

```
1. 先发送一张 QQ 商店表情包（插件会自动缓存）
2. 引用该表情包，输入：/快速 表情包 开心的表情
3. 插件自动从本地缓存获取图片，生成新表情包
```

> **注意**: 首次使用某张表情包时需要先发送一次让插件缓存，之后就可以随时引用使用了。
